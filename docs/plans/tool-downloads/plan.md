...
**Second addendum (same day, post-implementation):** the user asked for a single command, run from the game directory, that launches Tracy and runs the game together. Rather than hardcoding Last Beacon's `profiling` Cargo feature name into the engine, added a generic `--features <name1,name2>` flag to `foundation-build build/run/package`, additive on top of whatever Foundation's own configuration already computes (`dev-tools`, `editor`) -- reusable by any Foundation game for any of its own Cargo features, not profiling-specific. The actual one-command convenience (`scripts\profile.cmd`, launch Tracy then run the game with `--features profiling`) is Last Beacon-specific glue and lives in the root repository, not the engine, since knowing to combine "the profiling feature is named `profiling`" with "the tool is named `tracy`" is game policy, not engine policy. Validated for real: `scripts\profile.cmd` launched Tracy and built+ran the actual game with profiling enabled, confirmed working end to end by the user.
